...
#
#   4. `vllm.v1.worker.gpu.sample.output._pack_sampling_mask_kernel` /
#      `output._compact_sampling_mask_kernel` and `SamplingMaskTensors.from_logits`
#    Why:
#       (1) Both upstream `from_logits` launchers hard-code `BLOCK_SIZE=8192`,
...
#       (2) Release lane (v0.29.0) only: the upstream `_pack_sampling_mask_kernel`
#       sums an int1 tensor (`tl.sum(keep)`) before casting to int32, which
#       native CUDA Triton upcasts automatically. triton-ascend does not upcast,
#       so `counts` is truncated to 0/1 and the sampling-mask replay (mask_replay)
#       returns wrong candidate sets on NPU. The main lane already casts `keep`
#       before the reduction.
#    How:
#       Provide Ascend copies of both kernels in
#       `vllm_ascend/ops/triton/v2/sample/pack_sampling_mask.py` and rebind
#       `SamplingMaskTensors.from_logits` to the matching launcher, both
#       launching with the reduced `SAMPLING_MASK_BLOCK_SIZE` (1024) instead of
...
#    Future Plan:
#       Remove once triton-ascend supports the upstream BLOCK_SIZE and (release
#       lane) upcasts int1 reductions, or vLLM adds a backend-dispatch mechanism
#       for this kernel.
#
